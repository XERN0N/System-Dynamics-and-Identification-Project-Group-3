...
data_secondary = pd.read_csv("Sorted timeseries/Free/1D/Calibrated/weak_axis_data/timeseries_gr3_FirstSoft.txt")
data_primary = pd.read_csv("Sorted timeseries/Free/1D/Calibrated/weak_axis_data/timeseries_gr3_FirstSoft.txt")
...
with model_primary.added_forces({8: lambda t: [0, 130, 0, 0, 0, 0]}):
    initial_condition_solver_primary = Static(model_primary).solve()

solution_primary = Newmark(model_primary, initial_condition_solver_primary, end_time, time_increment).solve().solution

data_secondary = pd.read_csv("Sorted timeseries/Free/1D/Calibrated/strong_axis_data/timeseries_gr3_FirstStiff.txt")
